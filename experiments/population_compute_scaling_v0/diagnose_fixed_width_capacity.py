"""Development-only fixed-width relay-2 capacity diagnostic.

Train independent repaired RelayPopulationModel checkpoints at one fixed active population each.
This separates per-width learnability from mixed-width curriculum effects. It is not Gate-v0,
does not compare learned parameter scaling, and never opens confirmation data.
"""
...
WIDTHS = (16, 64, 256)
TRAIN_BATCH_BY_WIDTH = {16: 16, 64: 4, 256: 1}
TRAINING_SEED_BASE = 13_000_000_000
HELDOUT_SEED_BASE = 14_000_000_000
...
def main(argv: list[str] | None = None) -> int:
    ...
    if difficulty.name != "relay-2":
        raise RuntimeError("fixed-width diagnostic expects relay-2 as first difficulty")
    thresholds = relay_scope_thresholds(difficulty)
    if any(width not in thresholds for width in WIDTHS):
        raise RuntimeError("diagnostic widths must remain frozen relay-2 scope thresholds")

    device = torch.device(args.device)
    results = tuple(
        _train_and_evaluate_width(
            width=width,
            training_seed=700 + index,
            steps=args.steps,
    ...
    parameter_counts = {result.learned_parameter_count for result in results}
    if len(parameter_counts) != 1:
        raise RuntimeError("fixed-width diagnostic changed learned parameter count")

    payload = {
        "diagnostic": "relay-fixed-width-capacity-v0",
        "architecture": "compositional-node-messages+hop-local-worker-state",
        ...
        "interpretation_contract": {
            "all_fixed_widths_learn": "mixed-width curriculum/normalization is the remaining blocker",
            "capability_degrades_with_width": "population aggregation/selectivity is the remaining blocker",
            "width16_fails": "large-width aggregation/selectivity already fails near the first larger population",
        },
...
def _train_and_evaluate_width(
    *,
    ...
    initial_fingerprint = model.parameter_fingerprint()
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4)
    losses: list[float] = []

    model.train()
    training_base = TRAINING_SEED_BASE + width * 10_000_000
    for step in range(steps):
        worlds = _threshold_worlds(
            base_seed=training_base,
        ...
        if not torch.isfinite(loss):
            raise RuntimeError(f"width {width} produced non-finite training loss")
        optimizer.zero_grad(set_to_none=True)
        loss.backward()
        optimizer.step()
        losses.append(float(loss.detach().item()))

    trained_fingerprint = model.parameter_fingerprint()
    if trained_fingerprint == initial_fingerprint:
        raise RuntimeError(f"width {width} training did not change the checkpoint")
    if model.trainable_parameter_count() != parameter_count:
        raise RuntimeError(f"width {width} training changed learned parameter count")

    heldout_base = HELDOUT_SEED_BASE + width * 10_000_000
    heldout_worlds = _threshold_worlds(
        base_seed=heldout_base,
        offset=0,
    ...
    if model.parameter_fingerprint() != trained_fingerprint:
    ...
    return FixedWidthResult(
        ...
        parameter_fingerprint=trained_fingerprint,
        first_loss=losses[0],
...
def _evaluate(
    ...
    *,
    ...
    correct_bits = 0
    total_bits = 0
    evaluation_batch_size = max(1, 256 // width)
    with torch.inference_mode():
        for offset in range(0, len(worlds), evaluation_batch_size):
            world_batch = worlds[offset : offset + evaluation_batch_size]
            ...
            if not bool(torch.all(batch.information_complete).item()):
                raise RuntimeError(f"width {width} heldout batch is not information-complete")
            output = model(
                batch,
    ...
    while len(selected) < count:
        if thresholds[seed % len(thresholds)] == threshold:
            ...
            if world.scope_threshold != threshold:
                raise RuntimeError("fixed-width threshold selection drifted")
