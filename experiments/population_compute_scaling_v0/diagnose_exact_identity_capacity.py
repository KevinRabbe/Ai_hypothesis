"""Development-only fixed-width capacity diagnostic after exact relay identity repair.

Train fresh checkpoints at 4/16/64/256 workers using #64's corrected protocol:
raw candidate node projection -> shared reducer tanh -> exact bounded next-query representation.
This reopens no confirmation data and is not Gate-v0.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if difficulty.name != "relay-2" or any(width not in thresholds for width in WIDTHS):
        raise RuntimeError("exact-identity diagnostic no longer matches frozen relay-2 widths")
    ...
    if len({result.learned_parameter_count for result in results}) != 1:
        raise RuntimeError("learned parameter count changed across fixed widths")

    payload = {
        "diagnostic": "relay-exact-identity-capacity-v0",
    ...
    while len(selected) < count:
        if thresholds[seed % len(thresholds)] == threshold:
            ...
            if world.scope_threshold != threshold:
                raise RuntimeError("exact-identity threshold selection drifted")
