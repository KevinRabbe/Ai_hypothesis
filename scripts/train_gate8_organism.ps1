...
try {
    $wrapperSmoke = $env:GATE8_ORGANISM_TRAINING_WRAPPER_SMOKE -eq "1"
    $status = @(git status --porcelain)
    if ($LASTEXITCODE -ne 0) { throw "Could not read Git working-tree status." }
    if ($status.Count -ne 0) {
        $status | ForEach-Object { Write-Host $_ }
        throw "Gate8 organism training requires a clean Git working tree."
    }

    $branchOutput = git branch --show-current
    if ($LASTEXITCODE -ne 0) { throw "Could not resolve current Gate8 training branch." }
    $branch = if ($null -eq $branchOutput) { "" } else { "$branchOutput".Trim() }
    if ($branch.Length -eq 0 -and $wrapperSmoke -and $env:GITHUB_HEAD_REF) {
        $branch = $env:GITHUB_HEAD_REF
    }
    if ($branch -ne "agent/gate8-organism-training-execution-v0") {
        throw "Gate8 organism training must run from agent/gate8-organism-training-execution-v0."
    }

    $head = (git rev-parse HEAD).Trim()
    if ($LASTEXITCODE -ne 0 -or $head.Length -ne 40) {
        throw "Could not resolve exact Gate8 training Git head."
    }

    if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
        $OutputRoot = "F:\gate8_organism_training_seed${Seed}_v0"
    }
    ...
    if ($resolvedOutputRoot.StartsWith($resolvedRepoRoot + "\", [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Gate8 training output must be outside the Git repository."
    }
    if (Test-Path -LiteralPath $resolvedOutputRoot) {
        throw "Gate8 training output already exists: $resolvedOutputRoot"
    }
    New-Item -ItemType Directory -Path $resolvedOutputRoot | Out-Null

    $trainingRoot = Join-Path $resolvedOutputRoot "training"
    $resultPath = Join-Path $trainingRoot "gate8-organism-training-result.json"
    ...
    $runConfig = [ordered]@{
        experiment = "gate8-organism-training-execution-v0"
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-8 ORGANISM TRAINING"
    Write-Host "============================================================"
    ...
    if ($wrapperSmoke) {
        Write-Host "Gate8 training wrapper smoke completed before Torch, CUDA, worlds, optimizer or checkpoints."
        return
    }
...
if not torch.cuda.is_available():
    raise SystemExit("CUDA is required for Gate8 organism training")
print(f"gpu={torch.cuda.get_device_name(0)}")
'@
    $preflight | python -
    if ($LASTEXITCODE -ne 0) {
        throw "Gate8 CUDA preflight failed."
    }

    python scripts/train_gate8_organism.py `
        --seed $Seed `
        --device cuda `
        --output-root $trainingRoot
    if ($LASTEXITCODE -ne 0) {
        throw "Gate8 organism training failed. Preserve the complete output root for diagnosis."
    }
    if (-not (Test-Path -LiteralPath $resultPath -PathType Leaf)) {
        throw "Gate8 training result is missing: $resultPath"
    }

    $result = Get-Content -Raw $resultPath | ConvertFrom-Json
    if ($result.seed -ne $Seed) { throw "Gate8 training result seed drifted." }
    if ($result.learned_parameter_count -ne 19649) {
        throw "Gate8 training result parameter count drifted."
    }
    if ($result.training_worlds -ne 262144 -or $result.optimizer_steps -ne 1024) {
        throw "Gate8 training result schedule drifted."
    }
    if ($result.training_performed -ne $true -or $result.validation_performed -ne $true) {
        throw "Gate8 training result lacks completed train/validation evidence."
    }
    if ($result.scientific_test_worlds_generated -ne $false) {
        throw "Gate8 training crossed the scientific-test boundary."
    }
    if (
        ...
        throw "Gate8 training crossed the reference-model boundary."
    }
    if (@($result.checkpoint_candidates).Count -ne 4) {
        throw "Gate8 training did not preserve all four checkpoint candidates."
    }
    if (-not (Test-Path -LiteralPath (Join-Path $trainingRoot "selected-checkpoint.pt") -PathType Leaf)) {
        throw "Gate8 selected checkpoint is missing."
    }
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-8 organism training complete"
    Write-Host "============================================================"
