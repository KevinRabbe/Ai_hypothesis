param(
    ...
    [string]$OutputRoot = "F:\gate4_adaptive_activation_confirmation_v0",
...
function Assert-RequiredLeaf {
    ...
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "Required Gate-4 confirmation file is missing: $Path"
    }
}
...
try {
    if (-not $IdleMachineAttested) {
        throw "Gate-4 confirmation requires explicit -IdleMachineAttested."
    }
    ...
    if ($status.Count -ne 0) {
        ...
        throw "Gate-4 confirmation requires a clean Git working tree."
    }

    $branch = (git branch --show-current).Trim()
    if ($LASTEXITCODE -ne 0 -or $branch -ne "agent/gate4-confirmation-v0") {
        throw "Gate-4 confirmation must run from agent/gate4-confirmation-v0."
    }

    $head = (git rev-parse HEAD).Trim()
    if ($LASTEXITCODE -ne 0 -or $head.Length -ne 40) {
        throw "Could not resolve exact Gate-4 confirmation Git head."
    }

    $resolvedOutputRoot = [System.IO.Path]::GetFullPath($OutputRoot)
    if (Test-Path -LiteralPath $resolvedOutputRoot) {
        throw "Gate-4 confirmation output root already exists: $resolvedOutputRoot"
    }
    New-Item -ItemType Directory -Path $resolvedOutputRoot | Out-Null

    $scienceRoot = Join-Path $resolvedOutputRoot "science"
    $resultPath = Join-Path $scienceRoot "gate4-adaptive-activation-confirmation.json"
    ...
    $runConfig = [ordered]@{
        protocol = "gate4-adaptive-activation-confirmation-v0"
    ...
    $wrapperSmoke = $env:GATE4_CONFIRMATION_WRAPPER_SMOKE -eq "1"

    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-4 v0 ADAPTIVE ACTIVATION - CONFIRMATION"
    Write-Host "============================================================"
    ...
    if ($wrapperSmoke) {
        Write-Host "Gate-4 confirmation wrapper smoke completed before checkpoint/CUDA/scientific execution."
        return
    }
...
'@
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Gate-4 confirmation CUDA preflight failed."
    }

    python -m ai_hypothesis.population_compute.run_gate4_confirmation `
        --output-root $scienceRoot `
        --checkpoint0 $Checkpoint0 `
        --checkpoint1 $Checkpoint1 `
        --checkpoint2 $Checkpoint2
    if ($LASTEXITCODE -ne 0) {
        throw "Gate-4 confirmation evaluator failed. Preserve the output root for diagnosis."
    }
    ...
    Write-Host ""
    Write-Host "Independently auditing Gate-4 confirmation artifact..."
    python -m ai_hypothesis.population_compute.analyze_gate4_confirmation `
        $resultPath `
        --output $auditPath
    if ($LASTEXITCODE -ne 0) {
        throw "Independent Gate-4 confirmation audit rejected the artifact. Preserve all output."
    }
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-4 adaptive-activation confirmation complete"
    Write-Host "============================================================"
    Write-Host "Artifact valid:        $($audit.artifact_valid)"
    Write-Host "Confirmation outcome:  $($audit.confirmation_outcome)"
    Write-Host "Scientific status:     FINAL_GATE4_CONFIRMATION_EVIDENCE"
