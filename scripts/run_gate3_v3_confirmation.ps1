param(
    ...
    [string]$OutputRoot = "F:\gate3_v3_generation_pressure_confirmation_v0",
...
function Assert-RequiredLeaf {
    ...
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "Required Gate-3 v3 confirmation file is missing: $Path"
    }
}
...
try {
    if (-not $IdleMachineAttested) {
        throw "Gate-3 v3 confirmation requires explicit -IdleMachineAttested."
    }
    ...
    if ($status.Count -ne 0) {
        ...
        throw "Gate-3 v3 confirmation requires a clean Git working tree."
    }

    $branch = (git branch --show-current).Trim()
    if ($LASTEXITCODE -ne 0 -or $branch -ne "agent/gate3-v3-confirmation-v0") {
        throw "Gate-3 v3 confirmation must run from agent/gate3-v3-confirmation-v0."
    }

    $head = (git rev-parse HEAD).Trim()
    if ($LASTEXITCODE -ne 0 -or $head.Length -ne 40) {
        throw "Could not resolve exact Gate-3 v3 confirmation Git head."
    }

    $resolvedOutputRoot = [System.IO.Path]::GetFullPath($OutputRoot)
    if (Test-Path -LiteralPath $resolvedOutputRoot) {
        throw "Gate-3 v3 confirmation output root already exists: $resolvedOutputRoot"
    }
    New-Item -ItemType Directory -Path $resolvedOutputRoot | Out-Null

    $scienceRoot = Join-Path $resolvedOutputRoot "science"
    $resultPath = Join-Path $scienceRoot "gate3-v3-confirmation.json"
    ...
    $runConfig = [ordered]@{
        protocol = "gate3-v3-generation-pressure-confirmation-v0"
        scientific_status = "CONFIRMATION"
    ...
    $wrapperSmoke = $env:GATE3_V3_CONFIRMATION_WRAPPER_SMOKE -eq "1"

    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-3 v3 GENERATION PRESSURE - CONFIRMATION"
    Write-Host "============================================================"
    Write-Host "Git head: $head"
    Write-Host "Output:   $resolvedOutputRoot"
    Write-Host "Training: NONE - frozen checkpoints only"
    Write-Host "Worlds:   512 untouched confirmation worlds"
    Write-Host "Scheduler/work: unchanged from qualified v3 development"
    Write-Host "Confirmation is OPEN under the frozen protocol."
    Write-Host ""

    if ($wrapperSmoke) {
        Write-Host "Gate-3 v3 confirmation wrapper smoke completed before checkpoint/CUDA/scientific execution."
        return
    }
...
'@
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Gate-3 v3 confirmation CUDA preflight failed."
    }

    python -m ai_hypothesis.population_compute.run_gate3_v3_confirmation `
        --output-root $scienceRoot `
        --checkpoint0 $Checkpoint0 `
        --checkpoint1 $Checkpoint1 `
        --checkpoint2 $Checkpoint2
    if ($LASTEXITCODE -ne 0) {
        throw "Gate-3 v3 confirmation evaluator failed. Preserve the output root for diagnosis."
    }
    ...
    Write-Host ""
    Write-Host "Independently auditing Gate-3 v3 confirmation artifact..."
    python -m ai_hypothesis.population_compute.analyze_gate3_v3_confirmation `
        $resultPath `
        --output $auditPath
    if ($LASTEXITCODE -ne 0) {
        throw "Independent Gate-3 v3 confirmation audit rejected the artifact. Preserve all output."
    }
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-3 v3 confirmation complete"
    Write-Host "============================================================"
