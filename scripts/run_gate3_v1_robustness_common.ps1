...
function Assert-RequiredLeaf {
    ...
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "Gate-3 v1 robustness runner did not produce required file: $Path"
    }
}
...
try {
    if (-not $IdleMachineAttested) {
        throw "Gate-3 v1 robustness requires explicit -IdleMachineAttested."
    }
    ...
    if ($status.Count -ne 0) {
        ...
        throw "Gate-3 v1 robustness requires a clean Git working tree."
    }

    $branch = (git branch --show-current).Trim()
    if ($LASTEXITCODE -ne 0 -or $branch -ne "agent/gate3-v1-seed0-analysis") {
        throw "Gate-3 v1 robustness must run from agent/gate3-v1-seed0-analysis."
    }

    $head = (git rev-parse HEAD).Trim()
    if ($LASTEXITCODE -ne 0 -or $head.Length -ne 40) {
        throw "Could not resolve exact Gate-3 v1 robustness Git head."
    }

    $resolvedOutputRoot = [System.IO.Path]::GetFullPath($OutputRoot)
    if (Test-Path -LiteralPath $resolvedOutputRoot) {
        throw "Gate-3 v1 robustness output root already exists: $resolvedOutputRoot"
    }
    New-Item -ItemType Directory -Path $resolvedOutputRoot | Out-Null

    $scienceRoot = Join-Path $resolvedOutputRoot "science"
    $resultPath = Join-Path $scienceRoot "gate3-v1-robustness-seed-$TrainingSeed.json"
    $checkpointPath = Join-Path $scienceRoot "gate3-v1-robustness-seed-$TrainingSeed-checkpoint.pt"
    $runtimePath = Join-Path $scienceRoot "runtime.json"
    $auditPath = Join-Path $resolvedOutputRoot "robustness-audit.json"
    ...
    $runConfig = [ordered]@{
        ...
        execution = "robustness-development"
        scientific_status = "ROBUSTNESS_DEVELOPMENT_ONLY_NO_GATE_VERDICT"
    ...
    $wrapperSmoke = $env:GATE3_V1_ROBUSTNESS_WRAPPER_SMOKE -eq "1"
...
'@
        ...
        if ($LASTEXITCODE -ne 0) { throw "Gate-3 v1 robustness CUDA preflight failed." }
    }

    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-3 v1 ROBUSTNESS DEVELOPMENT - seed $TrainingSeed"
    Write-Host "============================================================"
    ...
    if ($wrapperSmoke) {
        Write-Host "Gate-3 v1 robustness wrapper smoke completed before scientific execution."
        return
    }

    $module = if ($TrainingSeed -eq 1) {
        "ai_hypothesis.population_compute.run_gate3_v1_robustness_seed1"
    }
    else {
        "ai_hypothesis.population_compute.run_gate3_v1_robustness_seed2"
    }

    python -m $module --output-root $scienceRoot
    if ($LASTEXITCODE -ne 0) {
        throw "Gate-3 v1 robustness science runner failed. Preserve the output root for diagnosis."
    }
    ...
    Write-Host ""
    Write-Host "Independently auditing Gate-3 v1 robustness artifact..."
    python -m ai_hypothesis.population_compute.analyze_gate3_v1_robustness `
        $resultPath `
        --output $auditPath
    if ($LASTEXITCODE -ne 0) {
        throw "Independent Gate-3 v1 robustness audit rejected the artifact. Preserve all output."
    }
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-3 v1 robustness seed $TrainingSeed complete"
    Write-Host "============================================================"
    Write-Host "Artifact valid:       $($audit.artifact_valid)"
    Write-Host "Development outcome:  $($audit.directional_outcome)"
    Write-Host "Scientific status:    ROBUSTNESS DEVELOPMENT ONLY - NO GATE VERDICT"
