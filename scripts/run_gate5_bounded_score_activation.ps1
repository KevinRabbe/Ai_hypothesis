param(
    ...
    [string]$OutputRoot = "F:\gate5_bounded_score_activation_development_v0",
...
function Assert-RequiredLeaf {
    ...
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "Required Gate-5 file is missing: $Path"
    }
}
...
try {
    if (-not $IdleMachineAttested) {
        throw "Gate-5 development requires explicit -IdleMachineAttested."
    }
    ...
    if ($status.Count -ne 0) {
        ...
        throw "Gate-5 development requires a clean Git working tree."
    }

    $branch = (git branch --show-current).Trim()
    if ($LASTEXITCODE -ne 0 -or $branch -ne "agent/gate5-bounded-score-activation-v0") {
        throw "Gate-5 development must run from agent/gate5-bounded-score-activation-v0."
    }

    $head = (git rev-parse HEAD).Trim()
    if ($LASTEXITCODE -ne 0 -or $head.Length -ne 40) {
        throw "Could not resolve exact Gate-5 Git head."
    }

    $resolvedOutputRoot = [System.IO.Path]::GetFullPath($OutputRoot)
    if (Test-Path -LiteralPath $resolvedOutputRoot) {
        throw "Gate-5 output root already exists: $resolvedOutputRoot"
    }
    New-Item -ItemType Directory -Path $resolvedOutputRoot | Out-Null

    $scienceRoot = Join-Path $resolvedOutputRoot "science"
    $resultPath = Join-Path $scienceRoot "gate5-bounded-score-activation-development.json"
    ...
    $runConfig = [ordered]@{
        protocol = "gate5-bounded-score-activation-v0"
    ...
    $wrapperSmoke = $env:GATE5_WRAPPER_SMOKE -eq "1"

    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-5 v0 BOUNDED SCORE VISIBILITY - DEVELOPMENT ONLY"
    Write-Host "============================================================"
    ...
    if ($wrapperSmoke) {
        Write-Host "Gate-5 wrapper smoke completed before checkpoint/CUDA/scientific execution."
        return
    }
...
'@
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Gate-5 CUDA preflight failed."
    }

    python -m ai_hypothesis.population_compute.run_gate5_bounded_score_activation `
        --output-root $scienceRoot `
        --checkpoint0 $Checkpoint0 `
        --checkpoint1 $Checkpoint1 `
        --checkpoint2 $Checkpoint2
    if ($LASTEXITCODE -ne 0) {
        throw "Gate-5 evaluator failed. Preserve the output root for diagnosis."
    }
    ...
    Write-Host ""
    Write-Host "Independently auditing Gate-5 development artifact..."
    python -m ai_hypothesis.population_compute.analyze_gate5_bounded_score_activation `
        $resultPath `
        --output $auditPath
    if ($LASTEXITCODE -ne 0) {
        throw "Independent Gate-5 audit rejected the artifact. Preserve all output."
    }
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-5 bounded-score development complete"
    Write-Host "============================================================"
