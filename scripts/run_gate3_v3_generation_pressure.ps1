param(
    ...
    [string]$OutputRoot = "F:\gate3_v3_generation_pressure_development_v0",
...
function Assert-RequiredLeaf {
    ...
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "Required Gate-3 v3 file is missing: $Path"
    }
}
...
try {
    if (-not $IdleMachineAttested) {
        throw "Gate-3 v3 development requires explicit -IdleMachineAttested."
    }
    ...
    if ($status.Count -ne 0) {
        ...
        throw "Gate-3 v3 development requires a clean Git working tree."
    }

    $branch = (git branch --show-current).Trim()
    if ($LASTEXITCODE -ne 0 -or $branch -ne "agent/gate3-v3-generation-pressure-v0") {
        throw "Gate-3 v3 development must run from agent/gate3-v3-generation-pressure-v0."
    }

    $head = (git rev-parse HEAD).Trim()
    if ($LASTEXITCODE -ne 0 -or $head.Length -ne 40) {
        throw "Could not resolve exact Gate-3 v3 Git head."
    }

    $resolvedOutputRoot = [System.IO.Path]::GetFullPath($OutputRoot)
    if (Test-Path -LiteralPath $resolvedOutputRoot) {
        throw "Gate-3 v3 output root already exists: $resolvedOutputRoot"
    }
    New-Item -ItemType Directory -Path $resolvedOutputRoot | Out-Null

    $scienceRoot = Join-Path $resolvedOutputRoot "science"
    $resultPath = Join-Path $scienceRoot "gate3-v3-generation-pressure-development.json"
    ...
    $runConfig = [ordered]@{
        protocol = "gate3-v3-generation-pressure-v0"
        scientific_status = "DEVELOPMENT_ONLY_NOT_ASSIGNED"
        confirmation_opened = $false
        training_performed = $false
        depth = 8
        hint_reliability = 0.70
        scheduled_parent_slots = 223
        active_child_lanes = 2
        recurrent_updates_per_child = 8
        learned_updates_per_world = 3568
        learned_parameter_count = 19649
        stable_capacities = @(16, 64, 256)
        frontier_controls = @("collapsed_L256", "reshuffled_L256")
        worlds = 256
        evaluation_batch_size = 64
        bootstrap_samples = 2000
        scheduler = "generation_synchronous"
        expected_depth7_preprune = 128
        expected_l64_depth7_retained = 64
        expected_l256_depth7_retained = 128
        expected_l64_final_productive = 64
        expected_l64_final_sink = 32
        expected_l256_final_productive = 96
        expected_l256_final_sink = 0
    ...
    $wrapperSmoke = $env:GATE3_V3_WRAPPER_SMOKE -eq "1"

    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-3 v3 GENERATION PRESSURE - DEVELOPMENT ONLY"
    Write-Host "============================================================"
    Write-Host "Git head: $head"
    Write-Host "Output:   $resolvedOutputRoot"
    Write-Host "Training: NONE - frozen checkpoints only"
    Write-Host "Scheduler: generation-synchronous"
    Write-Host "Confirmation remains CLOSED."
    Write-Host ""

    if ($wrapperSmoke) {
        Write-Host "Gate-3 v3 wrapper smoke completed before checkpoint/CUDA/scientific execution."
        return
    }
...
'@
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Gate-3 v3 CUDA preflight failed."
    }

    python -m ai_hypothesis.population_compute.run_gate3_v3_generation_pressure `
        --output-root $scienceRoot `
        --checkpoint0 $Checkpoint0 `
        --checkpoint1 $Checkpoint1 `
        --checkpoint2 $Checkpoint2
    if ($LASTEXITCODE -ne 0) {
        throw "Gate-3 v3 generation-pressure evaluator failed. Preserve the output root for diagnosis."
    }
    ...
    Write-Host ""
    Write-Host "Independently auditing Gate-3 v3 generation-pressure artifact..."
    python -m ai_hypothesis.population_compute.analyze_gate3_v3_generation_pressure `
        $resultPath `
        --output $auditPath
    if ($LASTEXITCODE -ne 0) {
        throw "Independent Gate-3 v3 audit rejected the artifact. Preserve all output."
    }
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-3 v3 generation-pressure development complete"
    Write-Host "============================================================"
