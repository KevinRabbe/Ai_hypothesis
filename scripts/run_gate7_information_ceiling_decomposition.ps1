...
function Assert-RequiredLeaf {
    ...
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "Required Gate-7 information-ceiling file is missing: $Path"
    }
}
...
try {
    if (-not $IdleMachineAttested) {
        throw "Gate-7 information-ceiling execution requires explicit -IdleMachineAttested."
    }

    $wrapperSmoke = $env:GATE7_INFORMATION_CEILING_WRAPPER_SMOKE -eq "1"
    $status = @(git status --porcelain)
    if ($LASTEXITCODE -ne 0) { throw "Could not read Git working-tree status." }
    if ($status.Count -ne 0) {
        $status | ForEach-Object { Write-Host $_ }
        throw "Gate-7 information-ceiling execution requires a clean Git working tree."
    }

    $branchOutput = git branch --show-current
    if ($LASTEXITCODE -ne 0) { throw "Could not resolve current information-ceiling branch." }
    $branch = if ($null -eq $branchOutput) { "" } else { "$branchOutput".Trim() }
    if ($branch.Length -eq 0 -and $wrapperSmoke -and $env:GITHUB_HEAD_REF) {
        $branch = $env:GITHUB_HEAD_REF
    }
    if ($branch -ne "agent/gate7-information-ceiling-decomposition-execution-v0") {
        throw "Gate-7 information-ceiling execution must run from agent/gate7-information-ceiling-decomposition-execution-v0."
    }

    $head = (git rev-parse HEAD).Trim()
    if ($LASTEXITCODE -ne 0 -or $head.Length -ne 40) {
        throw "Could not resolve exact information-ceiling Git head."
    }

    $resolvedOutputRoot = [System.IO.Path]::GetFullPath($OutputRoot)
    if (Test-Path -LiteralPath $resolvedOutputRoot) {
        throw "Gate-7 information-ceiling output root already exists: $resolvedOutputRoot"
    }
    ...
    $auditPath = Join-Path $resolvedOutputRoot "information-ceiling-audit.json"
    ...
    $runConfig = [ordered]@{
        protocol = "gate7-information-ceiling-decomposition-v0"
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-7 INFORMATION-CEILING DECOMPOSITION"
    Write-Host "============================================================"
    ...
    Write-Host "Worlds:            512 fresh worlds/checkpoint/N"
    ...
    if ($wrapperSmoke) {
        Write-Host "Information-ceiling wrapper smoke completed before checkpoints, CUDA or world generation."
        return
    }
...
'@
    ...
    if ($LASTEXITCODE -ne 0) { throw "Information-ceiling CUDA preflight failed." }

    python -m ai_hypothesis.population_compute.run_gate7_information_ceiling_decomposition `
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Information-ceiling evaluator failed. Preserve the complete output root for diagnosis."
    }
    ...
    Write-Host ""
    Write-Host "Independently auditing information-ceiling artifact..."
    python -m ai_hypothesis.population_compute.analyze_gate7_information_ceiling_decomposition `
        $resultPath `
        --output $auditPath
    if ($LASTEXITCODE -ne 0) {
        throw "Independent information-ceiling audit rejected the artifact. Preserve all output."
    }
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host " Gate-7 information-ceiling decomposition complete"
    Write-Host "============================================================"
