...
class SQLiteIndexedThreadState:
    ...
    def _sync_to(self, *, target_sequence: int | None, page_size: int) -> int:
        ...
        with self._lock:
            ...
            # Validate only after the requested tail is applied. Replay can contain a relation
            # whose target is created later in the same requested snapshot.
            with self._connection:
                for page in pages:
                    for event in page:
    ...
    def _apply_event(self, event: LedgerEvent) -> None:
        ...
        if event.event_type == "THREAD_CREATED":
            if bool(self._thread_row(thread_id)["created"]):
            ...
            self._connection.execute(
                "UPDATE thread_state_record SET last_worker_id = ?, last_attempt_sequence = ? WHERE thread_id = ?",
    ...
    def _apply_dependency(self, event: LedgerEvent, *, add: bool) -> None:
        ...
        for ordinal, dependency_id in enumerate(event.reference_ids):
            self._require_text_id("dependency_thread_id", dependency_id)
            if add:
                # Duplicate ADD keeps first insertion sequence/ordinal, matching _extend_unique.
                self._connection.execute(
                    """
                    INSERT OR IGNORE INTO thread_dependency(
                        thread_id, dependency_thread_id, sequence, ordinal
                    ) VALUES (?, ?, ?, ?)
                    """,
                    (event.thread_id, dependency_id, event.sequence, ordinal),
                )
            else:
                self._connection.execute(
    ...
    def _apply_fork(self, event: LedgerEvent) -> None:
        ...
        for ordinal, child_id in enumerate(event.reference_ids):
            self._require_text_id("child_thread_id", child_id)
            self._ensure_thread(child_id)
            self._connection.execute(
                """
                INSERT OR IGNORE INTO thread_fork(
                    parent_thread_id, child_thread_id, sequence, ordinal
                ) VALUES (?, ?, ?, ?)
                """,
                (parent_id, child_id, event.sequence, ordinal),
            )
    ...
    def _apply_merge(self, event: LedgerEvent) -> None:
        ...
        for ordinal, source_id in enumerate(event.reference_ids):
            ...
            self._connection.execute(
                """
                INSERT OR IGNORE INTO thread_merge(
                    source_thread_id, target_thread_id, sequence, ordinal
                ) VALUES (?, ?, ?, ?)
                """,
                (source_id, target_id, event.sequence, ordinal),
            )
    ...
    def _validate_graph_unlocked(self) -> None:
        ...
        for row in self._connection.execute(
            "SELECT thread_id, dependency_thread_id FROM thread_dependency ORDER BY sequence, ordinal"
        ).fetchall():
        ...
        for row in self._connection.execute(
            "SELECT parent_thread_id, child_thread_id, sequence FROM thread_fork ORDER BY sequence, ordinal"
        ).fetchall():
        ...
        for row in self._connection.execute(
            "SELECT source_thread_id, target_thread_id, sequence FROM thread_merge ORDER BY sequence, ordinal"
        ).fetchall():
    ...
    def _snapshot_all_unlocked(self) -> tuple[ProjectedState, ...]:
        rows = self._connection.execute(
            "SELECT * FROM thread_state_record WHERE created = 1 ORDER BY created_sequence, thread_id"
        ).fetchall()
    ...
    def _edge_targets(
        ...
        rows = self._connection.execute(
            ...
            f"ORDER BY sequence, ordinal, {target_column}",
            (source_id,),
        ).fetchall()
    ...
    def _thread_row(self, thread_id: str) -> sqlite3.Row:
        row = self._connection.execute(
            "SELECT * FROM thread_state_record WHERE thread_id = ?", (thread_id,)
        ).fetchone()
        if row is None:
    ...
    def _validate_relation_creation(
        created: Mapping[str, int], source_id: str, target_id: str, sequence: int, event_type: str
    ) -> None:
    ...
    def _create_schema(self) -> None:
        with self._connection:
            ...
            self._connection.execute(
                "CREATE INDEX IF NOT EXISTS idx_thread_dependency ON thread_dependency(thread_id, sequence, ordinal)"
            )
            self._connection.execute(
                "CREATE INDEX IF NOT EXISTS idx_thread_fork_parent ON thread_fork(parent_thread_id, sequence, ordinal)"
            )
            self._connection.execute(
                "CREATE INDEX IF NOT EXISTS idx_thread_fork_child ON thread_fork(child_thread_id, sequence, ordinal)"
            )
            self._connection.execute(
                "CREATE INDEX IF NOT EXISTS idx_thread_merge_target ON thread_merge(target_thread_id, sequence, ordinal)"
            )
    ...
    def _meta_optional(self, key: str) -> str | None:
        row = self._connection.execute(
            "SELECT value FROM thread_index_meta WHERE key = ?", (key,)
        ).fetchone()
    ...
    def _decode_state(cls, payload: str) -> dict[str, Any]:
        ...
        for key in ("references", "hypotheses", "contradictions", "open_questions"):
            items = value.get(key)
