...
The ledger remains the only source of truth. This sidecar index persists derived evidence
status/counts so hot integration queries advance from the ledger tail instead of replaying the
entire history every time. It may be deleted and rebuilt at any point.
"""
...
class SQLiteIndexedIntegrationTracker:
    ...
    def sync(self, *, page_size: int = 1000) -> int:
        """Apply only ledger events after the durable projection checkpoint."""
        ...
        with self._lock:
            ...
            while True:
                ...
                with self._connection:
                    ...
                    self._set_meta("revision", str(page[-1].sequence))
                    self._set_meta("checkpoint_event_id", page[-1].event_id)
    ...
    def overview(
        ...
        """Return scheduler-facing counts.

        ``events`` is accepted for duck-compatibility with ``IntegrationTracker.overview``.
        The index intentionally advances from the ledger tail instead of rescanning that
        supplied full-history tuple.
        """
        ...
        evidence_by_thread = {str(row["thread_id"]): row for row in evidence_rows}
        delta_by_thread = {
            str(row["thread_id"]): int(row["delta_count"]) for row in delta_rows
        }
        ...
        *,
        ...
        for row in rows:
            ...
            if event is None or event.event_type != "EVIDENCE_ADDED":
                raise RuntimeError("integration index references a missing evidence event")
            evidence_id = str(row["evidence_id"])
            if event.payload.get("evidence_id") != evidence_id:
                raise RuntimeError("integration index evidence identity no longer matches ledger")
    ...
    def _create_schema(self) -> None:
        with self._connection:
            ...
            self._connection.execute(
                ...
                ON integration_evidence(first_disposition_sequence, created_sequence, evidence_id)
                """
            )
            self._connection.execute(
                """
    ...
    def _apply_disposition(self, event: LedgerEvent) -> None:
        ...
        for evidence_id in event.reference_ids:
            ...
            if event.sequence < created_sequence:
                raise ValueError("evidence disposition precedes evidence creation")
            first_sequence = row["first_disposition_sequence"]
    ...
    def _apply_knowledge_delta(self, event: LedgerEvent) -> None:
        ...
        try:
            ...
            raise ValueError(f"duplicate durable knowledge delta ID {delta_id!r}") from error
