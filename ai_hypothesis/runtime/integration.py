...
class IntegrationTracker:
    def __init__(self, ledger: SQLiteResearchLedger, config: IntegrationBackpressureConfig | None = None) -> None:
        self.ledger = ledger
    ...
    def record_disposition(self, evidence_ids: Sequence[str], disposition: EvidenceDispositionKind, *, reason: str | None = None, thread_id: str | None = None) -> LedgerEvent:
        ...
        return self.ledger.append_event(event_type="INTEGRATION_DISPOSITION_RECORDED", thread_id=thread_id, reference_ids=resolved_ids, payload=payload)

    def record_knowledge_delta(self, delta: KnowledgeDelta) -> LedgerEvent:
        delta.validate()
        return self.ledger.append_event(event_type="KNOWLEDGE_DELTA_RECORDED", thread_id=delta.thread_id, reference_ids=(delta.delta_id, *delta.reference_ids), parent_event_ids=delta.causal_event_ids, payload={"delta_id": delta.delta_id, "kind": delta.kind, "summary": delta.summary})

    def snapshot(self, *, thread_id: str | None = None) -> IntegrationSnapshot:
        overview = self.overview(self.ledger.read_all_events())
        if thread_id is None:
            return overview.global_snapshot
        return overview.thread_snapshots.get(thread_id, self._empty_snapshot(overview.global_snapshot.revision))
            ...
            if event.event_type == "EVIDENCE_ADDED":
                ...
                if isinstance(evidence_id, str) and evidence_id:
                    ...
                    if event.thread_id is not None:
                        thread_evidence.setdefault(event.thread_id, {}).setdefault(evidence_id, event.sequence)
                ...
                if event.thread_id is not None:
                    thread_delta_counts[event.thread_id] = thread_delta_counts.get(event.thread_id, 0) + 1
        global_snapshot = self._build_snapshot(evidence_sequences=global_evidence, dispositioned=dispositioned, knowledge_delta_count=global_delta_count, revision=revision)
        thread_ids = set(thread_evidence) | set(thread_delta_counts)
        thread_snapshots = {thread_id: self._build_snapshot(evidence_sequences=thread_evidence.get(thread_id, {}), dispositioned=dispositioned, knowledge_delta_count=thread_delta_counts.get(thread_id, 0), revision=revision) for thread_id in thread_ids}
        thread_pressure = {thread_id: self._pressure_for_snapshot(snapshot) for thread_id, snapshot in thread_snapshots.items()}
        return IntegrationOverview(global_snapshot=global_snapshot, thread_snapshots=thread_snapshots, thread_pressure=thread_pressure, global_backpressured=self._is_snapshot_backpressured(global_snapshot))
    ...
    def pending_batch(self, *, limit: int, thread_id: str | None = None) -> IntegrationBatch:
        ...
        for event in events:
            ...
            if thread_id is not None and event.thread_id != thread_id:
                continue
            evidence_id = event.payload.get("evidence_id")
            if not isinstance(evidence_id, str) or not evidence_id or evidence_id in dispositioned:
                continue
            data = event.payload.get("data")
            records.append(PendingEvidence(evidence_id=evidence_id, event_id=event.event_id, sequence=event.sequence, thread_id=event.thread_id, kind=str(event.payload.get("kind", "UNKNOWN")), summary=str(event.payload.get("summary", "")), source_reference_ids=tuple(reference_id for reference_id in event.reference_ids if reference_id != evidence_id), strength=self._optional_float(event.payload.get("strength")), uncertainty=self._optional_float(event.payload.get("uncertainty")), data=dict(data) if isinstance(data, Mapping) else {}))
            if len(records) >= limit:
                break
        return IntegrationBatch(revision=revision, records=tuple(records))

    def project(self, events: Sequence[LedgerEvent], *, thread_id: str | None = None) -> IntegrationSnapshot:
    ...
    def _pressure_for_snapshot(self, snapshot: IntegrationSnapshot) -> float:
        return max(self._ratio(snapshot.backlog_count, self.config.max_backlog_count), self._ratio(snapshot.oldest_backlog_age_sequences, self.config.max_backlog_age_sequences))

    def _is_snapshot_backpressured(self, snapshot: IntegrationSnapshot) -> bool:
        return snapshot.backlog_count > self.config.max_backlog_count or snapshot.oldest_backlog_age_sequences > self.config.max_backlog_age_sequences

    @staticmethod
    def _build_snapshot(*, evidence_sequences: Mapping[str, int], dispositioned: set[str], knowledge_delta_count: int, revision: int) -> IntegrationSnapshot:
        backlog = tuple(evidence_id for evidence_id in evidence_sequences if evidence_id not in dispositioned)
        age = max(0, revision - min(evidence_sequences[evidence_id] for evidence_id in backlog)) if backlog else 0
        return IntegrationSnapshot(revision=revision, evidence_count=len(evidence_sequences), dispositioned_evidence_count=sum(1 for evidence_id in evidence_sequences if evidence_id in dispositioned), backlog_evidence_ids=backlog, knowledge_delta_count=knowledge_delta_count, oldest_backlog_age_sequences=age)

    @staticmethod
    def _empty_snapshot(revision: int) -> IntegrationSnapshot:
        return IntegrationSnapshot(revision=revision, evidence_count=0, dispositioned_evidence_count=0, backlog_evidence_ids=(), knowledge_delta_count=0, oldest_backlog_age_sequences=0)
