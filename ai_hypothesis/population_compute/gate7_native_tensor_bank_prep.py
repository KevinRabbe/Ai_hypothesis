...
_PRIORITY_MODULUS = 2_305_843_009_213_693_951
...
def swap_delete_gate7_native_parent(
    ...
    *,
    ...
    """In-place dense-prefix swap-delete; touches O(batch*state_width), not O(N)."""
    ...
    bank.states[rows, selected_positions] = bank.states[rows, last]
    bank.scores[rows, selected_positions] = bank.scores[rows, last]
    bank.heap_ids[rows, selected_positions] = bank.heap_ids[rows, last]
    bank.states[rows, last] = 0
    bank.scores[rows, last] = 0
    bank.heap_ids[rows, last] = 0
...
def append_gate7_native_children(
    ...
    *,
    ...
    """In-place append into the live prefix / one overflow scratch position."""
    ...
    bank.states[rows, first] = child_states[:, 0]
    bank.states[rows, second] = child_states[:, 1]
    bank.scores[rows, first] = child_scores[:, 0]
    bank.scores[rows, second] = child_scores[:, 1]
    bank.heap_ids[rows, first] = child_heap_ids[:, 0]
    bank.heap_ids[rows, second] = child_heap_ids[:, 1]
...
def prune_gate7_native_overflow(
    ...
    *,
    ...
    """In-place deterministic score-blind single-victim prune."""
    ...
    victim_states = bank.states[rows, victim]
    victim_scores = bank.scores[rows, victim]
    victim_heap = bank.heap_ids[rows, victim]
    last_states = bank.states[rows, last]
    last_scores = bank.scores[rows, last]
    last_heap = bank.heap_ids[rows, last]

    bank.states[rows, victim] = torch.where(overflow[:, None], last_states, victim_states)
    bank.scores[rows, victim] = torch.where(overflow, last_scores, victim_scores)
    bank.heap_ids[rows, victim] = torch.where(overflow, last_heap, victim_heap)

    bank.states[rows, last] = torch.where(
        overflow[:, None], torch.zeros_like(last_states), bank.states[rows, last]
    )
    bank.scores[rows, last] = torch.where(
        overflow, torch.zeros_like(last_scores), bank.scores[rows, last]
    )
    bank.heap_ids[rows, last] = torch.where(
        overflow, torch.zeros_like(last_heap), bank.heap_ids[rows, last]
    )
