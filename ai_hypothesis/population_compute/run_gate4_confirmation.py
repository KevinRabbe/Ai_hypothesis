"""Run the frozen Gate-4 adaptive-activation confirmation matrix.

No training occurs.  The runner verifies and reuses the exact three frozen checkpoints and evaluates
all three scheduler conditions on 512 untouched confirmation worlds/checkpoint.
"""
...
from .gate4_confirmation import (
    ...
    Gate4ConfirmationResult,
    build_gate4_confirmation_paired_summaries,
    evaluate_gate4_confirmation_condition,
)
...
def run_gate4_confirmation(
    *, output_root: Path, checkpoint_paths: tuple[Path, Path, Path]
) -> int:
    if output_root.exists():
        raise FileExistsError(f"Gate-4 confirmation output already exists: {output_root}")
    if not torch.cuda.is_available():
        raise RuntimeError("admitted Gate-4 confirmation runner requires CUDA")
    ...
    total_cells = len(GATE4_CONFIRMATION_CHECKPOINT_INDICES) * len(GATE4_CONFIRMATION_CONDITIONS)
    completed = 0

    print("Gate-4 v0 adaptive-activation CONFIRMATION", flush=True)
    print(f"CUDA device: {torch.cuda.get_device_name(0)}", flush=True)
    print("Training: NONE — reusing three frozen Gate-3 v1 checkpoints", flush=True)
    print(f"Worlds: {GATE4_CONFIRMATION_WORLD_COUNT} untouched confirmation worlds/checkpoint", flush=True)
    ...
    print("Confirmation execution is OPEN under the frozen Gate-4 protocol.", flush=True)

    for checkpoint_index, checkpoint_path in zip(
        GATE4_CONFIRMATION_CHECKPOINT_INDICES, checkpoint_paths, strict=True
    ):
        ...
        for mode in GATE4_CONFIRMATION_CONDITIONS:
            condition = evaluate_gate4_confirmation_condition(
                model,
                checkpoint_index=checkpoint_index,
                mode=mode,
                device="cuda",
    ...
    paired = build_gate4_confirmation_paired_summaries(conditions)
    result = Gate4ConfirmationResult(
        experiment_version=GATE4_CONFIRMATION_VERSION,
        scientific_status="FINAL_GATE4_CONFIRMATION_EVIDENCE_PENDING_AUDIT",
        confirmation_opened=True,
        training_performed=False,
        checkpoints=tuple(identities),
        world_count=GATE4_CONFIRMATION_WORLD_COUNT,
        evaluation_batch_size=GATE4_CONFIRMATION_EVAL_BATCH_SIZE,
        bootstrap_samples=GATE4_CONFIRMATION_BOOTSTRAP_SAMPLES,
        depth=GATE4_DEPTH,
    ...
    result_path = output_root / "gate4-adaptive-activation-confirmation.json"
    ...
    print(
        json.dumps(
            {
                "status": "GATE4_CONFIRMATION_DATA_COMPLETE",
