"""Frozen Gate-3 v1 robustness execution for training seeds 1 and 2 only.

This module is post-seed0 infrastructure. It preserves the seed-0 recipe exactly and changes only the
training RNG seed. Confirmation is unreachable. Public entrypoints bind the seed so no free seed knob
is exposed to admitted execution.
"""
...
def train_gate3_v1_robustness_with_progress(
    *,
    training_seed: int,
    ...
    config.validate()
    if training_seed not in GATE3_V1_ROBUSTNESS_SEEDS:
        raise ValueError("Gate-3 v1 robustness permits exactly training seeds 1 and 2")
    ...
    for step in range(config.steps):
        ...
        if not torch.isfinite(loss):
            raise RuntimeError("Gate-3 v1 robustness training produced non-finite loss")
...
def _checkpoint_payload(
    ...
    *,
    ...
    return {
        ...
        "scientific_status": f"FROZEN_GATE3_V1_ROBUSTNESS_SEED{summary.training_seed}",
...
def run_gate3_v1_robustness_seed(*, training_seed: int, output_root: Path) -> int:
    if training_seed not in GATE3_V1_ROBUSTNESS_SEEDS:
        raise ValueError("robustness execution permits exactly seeds 1 and 2")
    if output_root.exists():
        raise FileExistsError(f"Gate-3 v1 robustness output already exists: {output_root}")
    if not torch.cuda.is_available():
        raise RuntimeError("admitted Gate-3 v1 robustness runner requires CUDA")
    ...
    print(f"Gate-3 v1 robustness development seed {training_seed}", flush=True)
    ...
    model, training = train_gate3_v1_robustness_with_progress(
        training_seed=training_seed,
        config=config,
        device="cuda",
        progress=progress,
    )
    checkpoint_path = output_root / f"gate3-v1-robustness-seed-{training_seed}-checkpoint.pt"
    torch.save(_checkpoint_payload(model, summary=training, config=config), checkpoint_path)

    conditions = []
    total_cells = sum(len(GATE3_V1_RESERVE_CAPACITIES[d]) for d in GATE3_V1_DEPTHS) * len(Gate3V1ControlMode)
    completed = 0
    ...
    result_path = output_root / f"gate3-v1-robustness-seed-{training_seed}.json"
    ...
    print(
        json.dumps(
            {
                "status": "GATE3_V1_ROBUSTNESS_DEVELOPMENT_COMPLETE",
                "scientific_decision": "ROBUSTNESS_ONLY_NOT_ASSIGNED",
                "confirmation_opened": False,
                "training_seed": training_seed,
