"""Run the frozen Gate-5 v0 bounded score-visibility confirmation matrix."""
...
from .gate5_confirmation import (
    ...
    Gate5ConfirmationResult,
    build_gate5_confirmation_paired_summaries,
    evaluate_gate5_confirmation_condition,
)
...
def run_gate5_confirmation(
    *, output_root: Path, checkpoint_paths: tuple[Path, Path, Path]
) -> int:
    if output_root.exists():
        raise FileExistsError(f"Gate-5 confirmation output already exists: {output_root}")
    if not torch.cuda.is_available():
        raise RuntimeError("admitted Gate-5 confirmation runner requires CUDA")
    ...
    total_cells = len(GATE5_CONFIRMATION_CHECKPOINT_INDICES) * len(GATE5_CONFIRMATION_CONDITIONS)
    completed = 0

    print("Gate-5 v0 bounded score-visibility CONFIRMATION", flush=True)
    print(f"CUDA device: {torch.cuda.get_device_name(0)}", flush=True)
    print("Training: NONE — reusing three frozen Gate-3 v1 checkpoints", flush=True)
    print(f"Worlds: {GATE5_CONFIRMATION_WORLD_COUNT} untouched confirmation worlds/checkpoint", flush=True)
    ...
    print("Primary confirmation: K16 learned routing + 5pp non-inferiority to global", flush=True)
    print("Strict bounded-score visibility runtime: unchanged from qualified development", flush=True)
    print("Confirmation execution is OPEN under the frozen Gate-5 protocol.", flush=True)

    for checkpoint_index, checkpoint_path in zip(
        GATE5_CONFIRMATION_CHECKPOINT_INDICES, checkpoint_paths, strict=True
    ):
        ...
        for mode in GATE5_CONFIRMATION_CONDITIONS:
            condition = evaluate_gate5_confirmation_condition(
                model,
                checkpoint_index=checkpoint_index,
                mode=mode,
                device="cuda",
    ...
    paired = build_gate5_confirmation_paired_summaries(conditions)
    result = Gate5ConfirmationResult(
        experiment_version=GATE5_CONFIRMATION_VERSION,
        ...
        evaluation_batch_size=GATE5_CONFIRMATION_EVAL_BATCH_SIZE,
        bootstrap_samples=GATE5_CONFIRMATION_BOOTSTRAP_SAMPLES,
        depth=GATE5_DEPTH,
    ...
    result_path = output_root / "gate5-bounded-score-activation-confirmation.json"
    ...
    print(
        json.dumps(
            {
                "status": "GATE5_CONFIRMATION_DATA_COMPLETE",
